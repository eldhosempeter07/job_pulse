    ...
    return Scaffold(
      ...
      body: _isLoading
        ...
        child: Column(
          ...
          children: _candidateIds
              .map((String candidateId) => CandidateWidget(candidateId))
              .toList(),
        ),
      ),
    );
...
class CandidateWidget extends StatelessWidget {
  ...
  const CandidateWidget(this.candidateId);
      ...
      builder: (BuildContext context, AsyncSnapshot<DataSnapshot> snapshot) {
        ...
        if (snapshot.hasData && snapshot.data!.value != null) {
          ...
          if (candidateData is Map<dynamic, dynamic>) {
            String profileImage = candidateData['profileImage'] ?? '';
            String name = candidateData['name'] ?? '';
            dynamic postsData = candidateData['posts'];
            if (postsData is Map<dynamic, dynamic>) {
              List<String> postKeys = postsData.keys.cast<String>().toList();
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: postKeys
                    .map((String postKey) => PostWidget(
                  postKey,
                  postsData[postKey],
                  profileImage,
                  name,
